...
function clamp(w){const out={};for(const k of keys){const b=base[k],cap=HARD[k]||Math.max(20,Math.abs(b)*2);out[k]=Math.max(-cap,Math.min(cap,Math.round(w[k])));if(POSITIVE_KEYS.includes(k)&&out[k]<0)out[k]=b;if(NEGATIVE_KEYS.includes(k)&&out[k]>0)out[k]=b;}return out;}
