...
if(!K._cassetteDeckRenderPatched){
  ...
  K.renderPlates=function(){
    ...
    const cards=h.map((id,i)=>{
      ...
      return '<div class="plateCard handCard'+(isActive?' active':'')+(powered?' powered':'')+'">'
    ...
    root.innerHTML='<div class="platesTitle"><span>カセット手札 '+h.length+'枚</span><span class="hintPlate">山札 '+d.length+' / 捨て札 '+x.length+sync+'</span></div><div class="plateRow handRow">'+cards+'</div>';
