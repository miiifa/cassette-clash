...
(function(K){
K.DECKS={p1:['pikachu','charmander','squirtle','bulbasaur','gengar','koko'],p2:['koko','gengar','rotom','greninja','mewtwo','deoxysD']};
...
function score(p){
  ...
  if((p.type==='move'||p.type==='deploy')&&p.n===K.TARGET.p2)return 90000000;
  ...
  if(p.type==='move'||p.type==='deploy'){
    ...
    if(K.ability&&K.ability(p.p,'jump'))s+=22000;
...
function think(){const ps=legal('p2');if(!ps.length)return null;let p=ps.find(x=>(x.type==='move'||x.type==='deploy')&&x.n===K.TARGET.p2);if(p)return p;const blocker=holdsGoal();if(blocker){const safe=ps.filter(x=>!(x.type==='move'&&x.p.id===blocker.id)&&score(x)>-10000000);return(safe.length?safe:ps).sort((a,b)=>score(b)-score(a))[0]||null;}const hg=goals('p1');p=ps.find(x=>(x.type==='move'||x.type==='deploy')&&x.n===K.TARGET.p1);if(p)return p;p=ps.find(x=>x.type==='battle'&&hg.some(g=>g.p&&g.p.id===x.d.id));if(p)return p;const safe=ps.filter(x=>score(x)>-10000000);return(safe.length?safe:ps).sort((a,b)=>score(b)-score(a))[0];}
